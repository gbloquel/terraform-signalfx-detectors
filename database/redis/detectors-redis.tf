...
resource "signalfx_detector" "evicted_keys" {
	...
	program_text = <<-EOF
		...
		detect(when(signal > ${var.evicted_keys_threshold_warning})).publish('WARN')
	EOF
...
resource "signalfx_detector" "expirations" {
	...
	program_text = <<-EOF
		...
		detect(when(signal > ${var.expirations_threshold_warning})).publish('WARN')
	EOF
...
resource "signalfx_detector" "keyspace_full" {
	...
	program_text = <<-EOF
		...
		detect(when(signal > ${var.keyspace_full_threshold_warning})).publish('WARN')
	EOF
...
resource "signalfx_detector" "memory_frag" {
	...
	program_text = <<-EOF
		...
		detect(when(signal > ${var.memory_frag_threshold_warning})).publish('WARN')
	EOF
...
resource "signalfx_detector" "rejected_connections" {
	...
	program_text = <<-EOF
		...
		detect(when(signal > ${var.rejected_connections_threshold_warning})).publish('WARN')
	EOF
